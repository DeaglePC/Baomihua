...
<p align="center">
  <img src="doc/demo.gif" alt="BaoMiHua Demo" />
</p>
...
**BaoMiHua** 和诸如 **Claude Code** 或 **Gemini CLI** 等终端 AI 工具有什么区别？

- **Claude Code / Gemini CLI（重型、多轮 Agent）**：
  这些工具通常被设计用于解决**复杂的软件工程任务**。它们能够读取你整个项目的代码库、跨文件修改代码、自动运行测试，并能在长对话中不断迭代。它们非常强大，但也显得十分“笨重”，启动和响应速度通常较慢。
- **BaoMiHua（轻量、单轮命令助手）**：
  这是一个**极速的单步命令生成助手**。它专为“快”和“日常终端操作”而生。它不会读取你的代码库，也不会和你寒暄聊天。你只需要用自然语言说出你**此刻立马**想执行的终端操作（比如查端口、解压文件、看内网 IP 等），它会瞬间翻译出最精准的 Shell 命令，并确保你在当前环境下安全地执行它。

#### 场景总结：
- **何时使用 Claude Code / Gemini CLI？**
  当你的诉求是：*“帮我重构一下项目的鉴权模块，并且顺便把单元测试写好。”*
- **何时使用 BaoMiHua？**
  当你的诉求是：*“找出占用 8080 端口的进程并且立刻强杀掉它”* 或者是 *“找出当前目录下体积最大的那个文件”*。
...
> **💡 提示**：由于 `()`（括号）、`< >`（重定向） 等在 Shell 中属于核心控制字符会被 Shell 优先拦截解析，如果你提问的自然语言中**包含这些特殊字符，请顺手用双引号把整句话包裹起来**：
> `?? "show my ip(local)"`
