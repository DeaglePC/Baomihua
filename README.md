...
> **💡 Tip**: Since characters like `()` (parentheses), `< >` (redirection), and `&` are core control characters in Shell, they will be intercepted and parsed by the Shell first. If your natural language query contains these special characters, **please wrap the entire sentence in double quotes**:
> `?? "show my ip(local)"`
